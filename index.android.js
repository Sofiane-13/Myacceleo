...
//function
RmplirTab() { if( (this.state.Input1 !== '') && (this.state.Input2 !== '')) { this.itemsRef.push({  Input1: this.state.Input1 , Input2: this.state.Input2 , });  this.setState({ Input1 : '' }) , this.setState({ Input2 : '' })   } } 
 Alert = () => { Alert.alert('Alerte'); };
renderRow(rowData) { return ( <TouchableHighlight onPress={() => this.removeTodo(rowData)}><View ><Text >Input1 : {rowData.text.Input1}</Text><Text >Input2 : {rowData.text.Input2}</Text></View></TouchableHighlight>  ); }
removeTodo(rowData) {  Alert.alert( ' Bravo ligne supprimée !');   this.itemsRef.child(rowData.id).remove();   }
render()
{
//Vue
return(
<View>
<Button title='Ajouter' onPress={() => this.RmplirTab()} onLongPress={() => this.Alert()} buttonStyle={ styles.Style1 } icon={{name: 'squirrel', type: 'RmplirTab', buttonStyle: styles.Alert }}  />
